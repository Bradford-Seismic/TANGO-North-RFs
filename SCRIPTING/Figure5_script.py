...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    ## subplot (a) ISC catalog in surface map
    
    joints_azi = line_azimuth(joints)
    
    prj = f'M{fig_x}c'
    fig.basemap(region = [-69, -64, -24, -22.5], projection = prj, frame = ['WseN', 'afg'])
    ...
    tecto_zones = gpd.read_file('../DATA/MAPPING/AndeanTectonicRegimes2.shp')
    fig.plot(data = tecto_zones[tecto_zones.Zone == 'Puna'], pen = '1p,black,6_6:6p')
...
"""   
N 4
S 0.40c i 0.3c gold 0.6p,black 0.65c Other Station
