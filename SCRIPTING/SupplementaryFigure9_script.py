# -*- coding: utf-8 -*-
"""
Created on Fri Nov 21 16:03:45 2025
...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  3 16:13:44 2025
...
with pygmt.config(FONT = '14p'):
    ...
    v_model = xr.open_dataset('../DATA/MAPPING/nc_models/BlendedModel_0.7APVC-ANT_0.3FWT_MantleSpec_Smoothed.nc')
    ...
    vpvs_model = xr.open_dataset('../DATA/MAPPING/nc_models/CentralAndes_Zoned_VpVs_Arc-1.85_Smoothed.nc')
    ...
    v_model = xr.open_dataset('../DATA/MAPPING/nc_models/BlendedModel_0.7APVC-ANT_0.3FWT_BigLVZ_RE_Smoothed.nc')
    ...
    vpvs_model = xr.open_dataset('../DATA/MAPPING/nc_models/CentralAndes_Zoned_VpVs_Arc-1.85_Puna-1.82_RE_Smoothed.nc')
...
G 0.07c
    ...
    with pygmt.config(FONT = '12p'):
         fig.legend(spec = spec,  position='jBL+w{}c+o0c/-3c'.format(fig_x))
 




    fig.shift_origin(xshift = '{}c'.format(fig_x + 2.5))

    mod = np.loadtxt('../DATA/MAPPING/nc_models/iasp91_mod_70km.tvel', skiprows = 2)
    
    fig.basemap(region = [3, 9, 0, depth], projection = 'X{}c/-{}c'.format(4, (fig_y+0.5)*3), frame = ['wSEn', 'xa2f1+lvelocity (km/s)', 'ya50f10g50+lDepth (km)'])
    
    fig.plot(x = mod[:,1], y = mod[:,0], pen = '1p,black,dashed')
    fig.plot(x = mod[:,2], y = mod[:,0], pen = '1p,blue')
...
S 0.20c - 1c 2p 1p,black,-- 1.2c vp
S 0.20c - 1c 2p 1p,blue 1.2c vs
G 0.07c
G 0.07c
...
fig.savefig('../FIGURES/S9_Extreme_vs_Comparison-Models.png', dpi = 300)
