# -*- coding: utf-8 -*-
"""
Created on Sun Nov 23 18:17:08 2025
...
with pygmt.config(FONT = '14p'):
    ...
    fig.basemap(region = [0, line_dist.max(), 0, depth], projection = 'X{}c/-{}c'.format(fig_x, fig_y), frame = ['Wsen', 'xa100f10g75+lProfile Distance (km)', 'ya50f10g75+lDepth (km)'])

    model = xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-2.5_GDW_RE.nc')
    model = model.assign_coords({'depth': model.depth.values * -1})
    ...
    fig.basemap(region = [0, line_dist.max(), 0, depth], projection = 'X{}c/-{}c'.format(fig_x, fig_y), frame = ['wsen', 'xg50', 'yg50'])
    ...
    model = xr.open_dataset('../DATA/MAPPING/nc_models//CCP_TANGO_North_G-2.5_GDW_IASP70km_RE.nc')
    model = model.assign_coords({'depth': model.depth.values * -1})
    model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
    model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
    model_slice.Amplitude.values = np.clip(model_slice.Amplitude.values, -0.5, 0.5)
...
G 0.07c
    ...
    with pygmt.config(FONT = '12p'):
         fig.legend(spec = spec,  position='jBL+w{}c+o0c/-2.5c'.format(fig_x))
...
fig.savefig('../FIGURES/S10_CCPMigration-Comparisons.png', dpi = 300)
