# -*- coding: utf-8 -*-
"""
Created on Wed Nov  5 00:08:38 2025
...
with pygmt.config(FONT = '12p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    model = xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-2.5_GDW_BAZ-290-360_RE.nc')
    ...
    model = xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-2.5_GDW_BAZ-90-180_RE.nc')
    model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
    model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
    model_slice.Amplitude.values = np.clip(model_slice.Amplitude.values, -0.5, 0.5)
    ...
    model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
    model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
    model_slice.Amplitude.values = np.clip(model_slice.Bin_Hits_std.values, 0, 100)


    grid_color = pygmt.makecpt(cmap = '../DATA/MAPPING/no_green.cpt',   series = [0, 100])
    fig.grdimage(grid = model_slice['Amplitude'], cmap = True)
    
    with pygmt.config(FONT_ANNOT_PRIMARY="24p", FONT_LABEL = '24p'):
        fig.colorbar(cmap = True, position = 'JMR+w{}+o0.25c/0c'.format(fig_y), frame = 'xa20f10+lHits within 8 km')
...
fig.savefig('../FIGURES/S6_CCP_BAZ_Differences.png', dpi = 300)
