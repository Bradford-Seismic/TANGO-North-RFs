# -*- coding: utf-8 -*-
"""
Created on Wed Oct 29 17:55:55 2025
...
def plot_topo(fig):
    ...
    fig.basemap(region = [0, dist_lim, 0,8], projection = 'X{}c/{}c'.format(fig_x, 1.5), frame = ['WsNe', 'xa100f10', 'ya5f1+e+lElev. (km)'])
    ...
    poly_int = Chile_Faults[Chile_Faults.F_system == 'ATFS'].union_all().intersection(line_gpd)
    int_dist = haversine(joints[j,1], joints[j,0], poly_int.get_coordinates().y.values[0], poly_int.get_coordinates().x.values[0]) + joints_dist[j]   
  
    int_elev = dem.values[closest_idx(dem['Distance_Along_Trend'], int_dist)] / 1000 * 3
    fig.plot(x = [int_dist, int_dist], y = [0, int_elev], pen = '1p,red,--')
   
    fig.text(x = int_dist, y = int_elev + 1500, text = 'ATFS', font='11p,Helvetica', )
  


    poly_int = Chile_Faults[Chile_Faults.F_system == 'CdSF'].union_all().intersection(line_gpd)
    for k in range(len(poly_int.get_coordinates())):
        int_dist = haversine(joints[j,1], joints[j,0], poly_int.get_coordinates().y.values[k], poly_int.get_coordinates().x.values[k]) + joints_dist[j]   
        
        int_elev = dem.values[closest_idx(dem['Distance_Along_Trend'], int_dist)]/1000 * 2
        fig.plot(x = [int_dist, int_dist], y = [0, int_elev], pen = '1p,red,--')

        
        fig.text(x = int_dist, y = int_elev + 1500, text = 'DFS', font='11p,Helvetica', )
...
ratio = dist_lim / 20 # km per figure cm
fig_y = depth/ratio
...
slab.coords['longitude'] = slab.x - 360; slab.coords['latitude'] = slab.y
slab = slab.swap_dims({'x': 'longitude', 'y': 'latitude'})

slab_slice = slab.interp(longitude = x, latitude = y, method = 'linear')
slab_slice = slab_slice.assign_coords({'Distance_Along_Trend': line_dist})
...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.basemap(region = [0, dist_lim, -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y),frame = ['Wsne', 'xafg+lProfile Distance (km)', 'ya20f10g+lDepth (km)'])
    ...
    fig.basemap(region = [0, dist_lim, -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y),frame = ['WSne', 'xafg50+lProfile Distance (km)', 'ya20f10g50+lDepth (km)'])
    ...
    slab_x = slab_slice['Distance_Along_Trend'].values
    slab_y = slab_slice.z.values
    fig.plot(x=  slab_x, y = slab_y, pen = '2p,black,--')


    eq_xyz = pd.read_csv('../DATA/MAPPING/Sippl_2018_Hypocenters.xyz')

    for i in range(len(joints) - 1):
        try:
            track = pygmt.project(data=eq_xyz, center=joints[i], endpoint=joints[i+1], width=[
                                  -24, 24], length=[0, joints_dist[i+1] - joints_dist[i]], unit=True)
            track = track.rename(columns={
                                 0: 'longitude', 1: 'latitude', 2: 'depth', 3: 'p', 4: 'q', 5: 'r', 6: 's'})
        
            fig.plot(x = track.p, y = -track.depth, style = 'c0.075c', fill = 'black')
        except:
            pass
...
S 0.20c i 0.3c magenta 0.2p,black 0.65c TANGO Broadband
S 0.20c i 0.3c cyan 0.4p,black 0.65c TANGO Node

S 0.3c kvolcano 0.30c red 0.75p,black 0.65c Holocene Volcano
S 0.05c - .5c - 1p,- 0.65c Slab 2.0 (Hayes, 2018)
S 0.10c c 0.15c black 0.2p,black 0.65c Hypocenters, Sippl et al (2018)
...
"""
    ...
    fig.legend(spec = spec,  position='jBL+w{}c+o0c/-3.5c'.format(fig_x))
