# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 00:28:00 2026
...
with xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-2.5_GDW_RE.nc') as model:
    model_slice = model.sel(depth = -57, method = 'nearest')

# truncate model_slice values to be within scale range
model_slice[metric].values[model_slice[metric].values > sca[1]] = sca[1] - 0.001
model_slice[metric].values[model_slice[metric].values < sca[0]] = sca[0] + 0.001

model_slice[metric].values[model_slice['Bin_Hits'] < 10] = np.nan
...
with fig.subplot(nrows=1, ncols =1, figsize = ('15c/15c')):
    with fig.set_panel(panel = 0):
        with pygmt.config(FONT="12p", FORMAT_GEO_MAP="ddd.xxF"):
            fig.basemap(region=[-68.5, -66, -23.5, -22.5], projection='M?')
            
            grid_color = pygmt.makecpt(cmap = '../DATA/MAPPING/no_green.cpt',   series = [sca[0], sca[1]])
            fig.grdimage(grid = model_slice[metric], cmap = True)
            with pygmt.config(FONT_ANNOT_PRIMARY="14p", FONT_LABEL = '16p'):
                
                freq = (sca[1] -  sca[0]) / 5
    
                fig.colorbar(cmap = grid_color, position = 'JMR+w6.5c+o0.5c/0c', frame = 'xa{}f{}+l{}'.format(np.max(sca), .1, metric))
        
        
        with pygmt.config(FONT="12p", FORMAT_GEO_MAP="ddd.xxF"):
            fig.basemap(region=[-68.5, -66, -23.5, -22.5], projection='M?', frame = 'a.5f1g.5')


        APVC = gpd.read_file('../DATA/MAPPING/APVC2.shp')
        Calderas = gpd.read_file('../DATA/MAPPING/Calderas.shp')
        fig.plot(data=APVC, pen = '0.7p,black', fill = 'pink', transparency = 50)
        fig.plot(data=Calderas, pen = '0.7p,black', fill = 'lightred', transparency = 50)


        fig.plot(x = stat_table.stlo, y =  stat_table.stla, style='i0.15c',fill = 'gold', pen='black')
        try:
            fig.plot(x = stat_table[stat_table.net == 'XN'].stlo, y =  stat_table[stat_table.net == 'XN'].stla, style='i0.15c',fill = 'cyan', pen='black')
        except:
            pass
        try:
            fig.plot(x = stat_table[stat_table.net == '1X'].stlo, y =  stat_table[stat_table.net == '1X'].stla, style='i0.15c',fill = 'cyan', pen='black')
        except:
            pass
        
        try:
            fig.plot(x = stat_table[stat_table.net == 'XM'].stlo, y =  stat_table[stat_table.net == 'XM'].stla, style='i0.15c',fill = 'magenta', pen='black')
        except:
            pass
        
        try:
            fig.plot(x = volcanoes.Longitude, y = volcanoes.Latitude, style='kvolcano/0.4c', fill = 'red', pen='0.5p,black')
        except:
            pass
...
"""   
N 3
S 0.20c i 0.3c gold 0.6p,black 0.65c Other Seismic Network
S 0.20c i 0.3c cyan 0.6p,black 0.65c TANGO Node
S 0.20c i 0.3c magenta 0.6p,black 0.65c TANGO Broadband


S 0.20c kvolcano 0.40c red 0.75p,black 0.65c Holocene Volcano
S 0.20c r 0.4/0.25 pink 0.8p,black 0.65c APVC Extent
...
with pygmt.config(FONT_ANNOT_PRIMARY="10p"):
    fig.legend(spec = spec,  position='jBL+w14c+o0c/2c')


fig.show()

fig.savefig('../FIGURES/S8_DepthSlice.png', dpi = 600)
