# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 11:57:53 2026
...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 29 19:35:54 2025
...
def plot_topo(fig):
    ...
    fig.basemap(region = [dist_min, dist_lim, 0,8], projection = 'X{}c/{}c'.format(fig_x, 1.5), frame = ['WsNe', 'xa100f10', 'ya5f1+e+lElev. (km)'])
...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    with xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-1.0_GDW_RE.nc') as model:
        ...
        # pull select RF profiles before clipping amplitude values
        RF1 = model_slice.interp(Distance_Along_Trend = 350, method = 'nearest')
        RF2 = model_slice.interp(Distance_Along_Trend = 425, method = 'nearest')
        RF3 = model_slice.interp(Distance_Along_Trend = 500, method = 'nearest')
    ...
    RFs = [RF1, RF2, RF3]
    for RF in RFs:          
        fig.plot(x = RF.Amplitude*25 + RF.Distance_Along_Trend, y = RF.depth, pen = '3p,green')
        fig.plot(x = RF.Amplitude*25 + RF.Distance_Along_Trend, y = RF.depth, pen = '1p,black')
...
"""   
N 4
S 0.40c i 0.3c gold 0.6p,black 0.65c Other Network
...
fig.savefig('../FIGURES/Fig5.5_ArcInterpretation_G-1.0.png', dpi = 600)


#%% Bottom Interpretation Frames
...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.basemap(region = [dist_min, dist_lim, -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y),frame = ['Wsne', 'xaf+lProfile Distance (km)', 'ya20f10+lDepth (km)'])
   
    
    fig.shift_origin(yshift = '-{}c'.format(fig_y + 0.5))
    fig.basemap(region = [dist_min, dist_lim, -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y),frame = ['WSne', 'xafg+lProfile Distance (km)', 'ya20f10g+lDepth (km)'])

    with xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-5.0_GDW_RE.nc') as model:
        model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
        model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
        model_slice.Amplitude.values = np.clip(model_slice.Amplitude.values, -0.25, 0.25)
...
# fig.savefig('../FIGURES/Fig5_ArcInterpretation_Base-Preferred.png', dpi = 300)





# IASP91 70 km crust Model
...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    fig.shift_origin(yshift = '-{}c'.format(2*fig_y + 0.5))
    fig.basemap(region = [dist_min, dist_lim, -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y),frame = ['WSne', 'xafg+lProfile Distance (km)', 'ya20f10g+lDepth (km)'])

    model = xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-5.0_GDW_IASP70km.nc')
    model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
    model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
    model_slice.Amplitude.values = np.clip(model_slice.Amplitude.values, -0.25, 0.25)
...
# fig.savefig('../FIGURES/Fig5_ArcInterpretation_Base-IASP70km.png', dpi = 300)
...
with pygmt.config(FONT = '14p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
    ...
    model = xr.open_dataset('../DATA/MAPPING/nc_models/CCP_TANGO_North_G-2.5_GDW_BigLVZ_Puna-1.82.nc')
    model_slice = model.interp(longitude = x, latitude = y, method='linear', kwargs={"fill_value": None})
    model_slice = model_slice.assign_coords({'Distance_Along_Trend': line_dist})
    model_slice.Amplitude.values = np.clip(model_slice.Amplitude.values, -0.25, 0.25)
...
# fig.savefig('../FIGURES/Fig5_ArcInterpretation_Base-BigLVZ.png', dpi = 300)
