...
with pygmt.config(FONT = '12p', MAP_ANNOT_OFFSET="10p", MAP_LABEL_OFFSET="10p"):
   ...
   ##### Plot Time Domain A-B
    ...
    itd_use = itd[itd.G == G][(itd.Accept == 'Auto QC Pass') | (itd.Accept == 'Man QC Pass')][itd.Region == region]
    ...
    fig.basemap(region = [0, line_dist.max(), -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y), frame = ['WSen', 'xa100f10+lProfile Distance (km)', 'ya50f10+lDepth (km)'])
    ...
    fig.basemap(region = [0, line_dist.max(), -depth, 0], projection = 'X{}c/{}c'.format(fig_x, fig_y), frame = ['WSen', 'xa100f10g50+lProfile Distance (km)', 'ya50f10g50+lDepth (km)'])
