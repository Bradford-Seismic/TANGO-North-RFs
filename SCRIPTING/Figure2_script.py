# -*- coding: utf-8 -*-
"""
Created on Sun Oct 26 23:15:08 2025
...
with pygmt.config(FONT = '14p'):
    ...
    fig.basemap(region = [0, line_dist.max(), 0, depth], projection = 'X{}c/-{}c'.format(fig_x, fig_y), frame = ['Wsen', 'xa100f10g50+lProfile Distance (km)', 'ya50f10g50+lDepth (km)'])
    
    with xr.open_dataset('../DATA/MAPPING/nc_models/{}'.format(shear_model)) as v_model:
    ...
    with pygmt.config(FONT_ANNOT_PRIMARY="24p", FONT_LABEL = '24p'):
        fig.colorbar(cmap = True, position = 'JMR+w{}+o0.25c/0c'.format(fig_y), frame = 'xa1f0.2+lvs (km/s)')
    ...
    fig.basemap(region = [0, line_dist.max(), 0, depth], projection = 'X{}c/-{}c'.format(fig_x, fig_y), frame = ['WSen', 'xa100f10g100+lProfile Distance (km)', 'ya50f10g100+lDepth (km)'])
   
    with xr.open_dataset('../DATA/MAPPING/nc_models/{}'.format(vpvs_model)) as vpvs_model:
    ...
    with pygmt.config(FONT_ANNOT_PRIMARY="24p", FONT_LABEL = '24p'):
        fig.colorbar(cmap = True, position = 'JMR+w{}+o0.25c/0c'.format(fig_y), frame = 'xa0.02f0.01+lvp/vs')





    fig.shift_origin(yshift = '-{}c'.format(fig_y + 7), xshift = '-2c')



    # EQ Map

    prj = "A-71.2/-20/100/{}c".format(fig_y+5)
    fig.coast(projection=prj, region = 'g', frame = 'g60', land="gray", shorelines = '0.5p, black', water = 'white')

    fig.plot(x = event_table.evlo[event_table.phase == 'P'], y = event_table.evla[event_table.phase == 'P'], style='c0.23c',fill = 'lightred', pen='0.6p,black')

    fig.plot(x = event_table.evlo[event_table.sense.str.contains('/XM', na = False)], y = event_table.evla[event_table.sense.str.contains('/XM', na = False)], style='c0.23c', pen='0.5p,black', fill = 'magenta' )
    fig.plot(x = event_table.evlo[event_table.sense.str.contains('/XN', na = False)], y = event_table.evla[event_table.sense.str.contains('/XN', na = False)], style='c0.23c', pen='0.5p,black', fill = 'cyan' )
    ...
    fig.shift_origin(xshift = '+10c', yshift = '0.5c')
    baz_hits = []
    ...
    fig.text(position = 'BR', offset = '-0.1c/0.2c', text = 'n = {}'.format(np.sum(baz_hits)), font = '14p,Helvetica,black', fill = 'white', pen = '1p,black')


    # Back-Azimuth Distribution
    
    fig.shift_origin(xshift = '+7c', yshift = '0c')
    baz_hits = []
    baz_bin = np.arange(0, 370, 10)
    for i in range(len(baz_bin-1)):
        hit_count = 0
    ...
    fig.shift_origin(xshift = '-3.7c', yshift = '4c')
    baz_hits = []
    baz_bin = np.arange(0, 370, 10)
    for i in range(len(baz_bin-1)):
        hit_count = 0
        for j in event_table.baz[(event_table.sense != '/XM') & (event_table.sense != '/XN') & (event_table.sense != '/XM/XN')].values:
            if (j > baz_bin[i]) and (j < baz_bin[i+1]):
                hit_count += 1
    ...
    fig.text(position = 'TR', offset = '-0.1c/-0.2c', text = 'n = {}'.format(np.sum(baz_hits)), font = '14p,Helvetica,black', fill = 'white', pen = '1p,black')

    spec = io.StringIO(
    """
N 1
G 0.05c
S 0.20c s 0.6c lightred 0.6p,black 0.65c Other Network
G 0.05c
S 0.20c s 0.6c magenta 0.6p,black 0.65c TANGO Broadband
G 0.10c
S 0.20c s 0.6c cyan 0.4p,black 0.65c TANGO Node
G 0.05c
    ...
    fig.legend( spec = spec, position='jTR+o-6c/-1c')
